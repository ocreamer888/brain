...
def load_gold(path: Path) -> list[dict[str, Any]]:
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if "query" not in obj or ("gold_files" not in obj and "gold_ids" not in obj):
            raise ValueError(f"{path}:{line_no}: missing query or gold_files/gold_ids")
        rows.append(obj)
    return rows
...
def score_one(
    ...
    # Prefer id-based matching (stable, resilient to file_path/tag changes);
    # fall back to vault-path matching for legacy gold entries.
    gold_ids = {str(x) for x in case.get("gold_ids", [])}
    gold_files = [normalize_path(x) for x in case.get("gold_files", [])]

    first_rank = 0
    for i, h in enumerate(results, 1):
        if gold_ids and str(h.get("id", "")) in gold_ids:
            first_rank = i
            break
        if gold_files:
            p = extract_path_from_hit(h)
            if p and p in gold_files:
                first_rank = i
                break
    ...
    if gold_ids:
        out["gold_ids"] = sorted(gold_ids)
    if gold_files:
        out["gold_files"] = gold_files
    return out
